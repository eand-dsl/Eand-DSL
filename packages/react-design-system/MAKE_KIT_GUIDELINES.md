...
- `SectionLinkButton({ surface: default|inverse, icon, disabled })` — the 40×40 icon-only link button that sits at the top-right of a section header. `Section` already renders one for you via `trigger="chevron"`; use this directly only when building a header by hand. `inverse` is the glass treatment for dark surfaces.
